...
import { View, ActivityIndicator, Pressable, Alert, TextInput } from 'react-native';
...
export default function OrderDetailsScreen() {
  ...
  async function submitRefund() {
    ...
    if (amount > remaining) {
      Alert.alert('Too much', ` Maximum refundable amount is $${remaining.toFixed(2)}`);
      return;
    }
  ...
  /* ---------- Ledger-safe totals ---------- */
  const itemsTotal = order?.items?.reduce((sum, i) => sum + i.line_total, 0) ?? 0;
  const refundedTotal = order?.refunds?.reduce((sum, r) => sum + r.amount, 0) ?? 0;
  ...
  return (
    <Screen className="gap-4">
      <AppText variant="title">Order #{order.id.slice(0, 8)}</AppText>

      <AppText>Status: {status?.toUpperCase()}</AppText>

      {isConfirmed && (
        <AppText className="font-semibold text-green-600">✔ This order is confirmed</AppText>
      )}

      {isCanceled && (
        <AppText className="font-semibold text-red-600">✖ This order was cancelled</AppText>
      )}

      <AppText>Placed: {new Date(order.created_at).toLocaleString()}</AppText>

      {/* ---------- Items ---------- */}
      <View className="gap-3 border-t pt-4">
        {(order.items ?? []).map((item) => (
          <View key={item.product_id} className="flex-row items-start justify-between">
            <View>
              <AppText>
            ...
            <AppText className="font-semibold">${item.line_total.toFixed(2)}</AppText>
          </View>
        ))}
      </View>
      ...
      {hasRefunds && (
        <View className="gap-3 border-t pt-4">
          ...
          {order.refunds.map((refund) => (
            <View key={refund.id} className="flex-row justify-between">
              <View>
              ...
              <AppText className="font-semibold text-red-600">-${refund.amount.toFixed(2)}</AppText>
            </View>
          ))}
        </View>
      )}
      ...
      {/* ---------- Totals ---------- */}
      <View className="gap-1 border-t pt-4">
        ...
        {hasRefunds && (
          <View className="flex-row justify-between">
            ...
            <AppText className="text-red-600">-${refundedTotal.toFixed(2)}</AppText>
          </View>
        )}

        <View className="flex-row justify-between pt-2">
          <AppText variant="title">Balance</AppText>
          <AppText variant="title">${remaining.toFixed(2)}</AppText>
        </View>

        {isFullyRefunded && (
          <AppText className="pt-2 font-semibold text-blue-600">
      ...
      {isPending && (
        <Pressable
          ...
          className={`mt-4 rounded-xl p-4 ${canceling ? 'bg-red-300' : 'bg-red-600'}`}>
      ...
      {refundOpen && (
        <View className="absolute inset-0 justify-center bg-black/40 px-6">
          <View className="gap-3 rounded-xl bg-white p-5">
            ...
            <AppText className="text-gray-500">Remaining balance: ${remaining.toFixed(2)}</AppText>
            ...
            <View className="flex-row justify-end gap-3 pt-3">
              ...
              <Pressable disabled={refunding} onPress={submitRefund}>
                <AppText className="font-semibold text-blue-600">
