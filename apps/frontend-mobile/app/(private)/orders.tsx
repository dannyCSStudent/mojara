...
export default function OrdersScreen() {
  ...
  return (
    <View className="flex-1 bg-white p-4">
      <Text className="mb-4 text-2xl font-bold">{isAdmin ? 'Orders' : 'My Orders'}</Text>
          ...
          {STATUSES.map((s) => {
            ...
            return (
              <Pressable
                key={s}
                onPress={() => setFilter(s)}
                className={`flex-1 rounded-lg py-2 ${isActive ? 'bg-white shadow' : ''}`}>
                <Text className="text-center font-medium">{s.toUpperCase()}</Text>
              </Pressable>
            );
          })}
        </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            <Text className="mt-1 font-medium">{item.status.toUpperCase()}</Text>
